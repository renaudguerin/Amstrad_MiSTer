...
| **B20-2: empty vector after a second acknowledge** (I1) | The synthetic two-ack diagnostic retains the source mismatch (DMA0/offset 4 versus empty fallback offset 0). The [production-bus investigation](plus/references/b20-production-ack-2026-09-23.md) connects T80 and the ASIC and locates the first structural divergence at the absent LK106/IC116 acknowledge shaping. No RTL correction is established. | Obtain the board-level pulse timing and instruction conditions, then derive a reachable failing vector before implementing compatibility behavior. Preserve vector stability within each acknowledge; do not generalize the source's bug-window result to every idle acknowledge. Original-hardware evidence remains open. |
...
| **B20-4: PRI phase relative to HSYNC** (I4) | [Source re-read and connected-module width sweep](plus/references/b20-pri-phase-2026-09-23.md): RTL follows revised Arnold's width-dependent shaped trailing edge, capped at 6 microseconds plus one master clock; widths 1–2 produce no shaped pulse/PRI. The fixed +10 microsecond source claim still conflicts. | Retain existing RTL pending original-hardware adjudication. Resolve short-width PRI and cross-line HSYNC/match behavior separately; the sweep did not test them against hardware. Preserve the hardware-backed nine-bit PRI **no-wrap** policy. Coordinate any interrupt edit with the Eerie owner. |
