...
## Where current priorities live

[current-status.md](current-status.md) is the handoff: each stream's position and its open
hardware residuals. The queue is section 8 of
[implementation-roadmap.md](implementation-roadmap.md). Completed items (B5, B7, B12, B15 and
the D5 boot-configuration note) are archived verbatim in
[archive/backlog-done.md](archive/backlog-done.md). Item numbers are stable; do not reuse
them.
...
| --- | --- | --- |
| **B20-1: live PPR writes** (I5) | Implemented at the source-model level: S22 p.10 diagram establishes next-iteration advancement, including same-value writes. Registered CPU write events now truncate the active interval; existing expiry cadence is preserved. | [Fail-first regs-to-DMA matrix and nine selected benches pass](plus/live-ppr-2026-09-22.md). Original-hardware validation and exact CPU-write/HSYNC phase remain open; no Sonic closure or synthesis claim. |
| **B20-2: empty vector after a second acknowledge** (I1) | [CRTC3 repair](investigations/hardware-runs/crtc3-demo-2026-09-25.md) implements schematic-derived IC116 shaping and offset4 for an empty repeated ASIC acknowledge within M1. Production-T80 controls and unchanged FlowLIB check pass; selected gate passes40 benches. | Cross-provider review and synthesized hardware acceptance pending. Preserve per-pulse vector stability; isolated empty acknowledge remains unspecified. Low-A13 FDC aliases follow schematic inference. |
| **B20-3: A13-dependent interrupt bug** (I2) | Shared board mechanism with B20-2: physical READY and A13 naturally split the low-address write case; high-address write and low-address HALT controls remain single-pulse. No opcode-specific T80 change. | Validate unchanged CRTC3 boot on the exact synthesized artifact; retain instruction-phase coverage and original-hardware limits in the [record](investigations/hardware-runs/crtc3-demo-2026-09-25.md). |
| **B20-4: PRI phase relative to HSYNC** (I4) | Ordinary monitor-trailing timing is preserved. The [CRTC3 connected probe](investigations/hardware-runs/crtc3-demo-2026-09-25.md) proves a missing line-entry event for raw HSYNC crossing into the matching line; repaired RTL passes fail-before/pass-after tests and restores settled Buddha/sphere replays. | Synthesized hardware acceptance pending, especially simultaneous HSYNC-fall/line-entry at R2=50. Short-width PRI and conflicting +10us source remain unresolved. Preserve nine-bit no-wrap. Eerie residual uses noncrossing HSYNC and is not explained by this fix. |
| **B20-5: DCSR read semantics** (I6) | [Register-boundary probe](plus/references/b20-dcsr-readback-2026-09-23.md) observes active-high live DMA flags in both IVR modes, cleared on automatic ack or explicit W1C. Original Amstrad prose supports active-high status; the automatic-mode freeze claim remains unresolved. No RTL change. | Repeat the recorded request/ack/write-clear matrix on original Plus hardware, including IVR changes while pending. Keep CPU DI/EI separate from peripheral flags. Resolve the conflicting claims before changing readback; preserve B19's already implemented raster-retention/provenance behavior. |
| **B20-6: DMA set/clear and STOP/write collisions** | [Connected regs/DMA observation](plus/references/b20-dma-collisions-2026-09-23.md) confirms level-wide W1C suppression, first-edge-only automatic clear, and STOP winning on the collision edge but a held enable write re-enabling on the next clock. The RTL comment already accurately describes clear dominance. | Retain RTL: no hardware collision priority or CPU write-sampling rule is established. Obtain a phase-controlled original-Plus trace before a behavioral repair; the probe does not establish production CPU reachability. Coordinate interrupt logic and hardware with Eerie. |
...
Why: the terminal-PAUSE rule (`190f4d3`, built as `a137d48`) lets a DMA channel run its next
instruction on the HSYNC that expires its last PAUSE tick, where the core waits one more line.
Its hardware failure predates the cartridge-stall fix. In scratch Sonic traces the list period
...
1. Re-apply the `rtl/plus/asic_dma.v` change from `190f4d3` on current master, adapted to B20-1
   live PPR so the prescaled terminal-edge condition stays consistent with live PPR writes.
   Restore its `asic_dma_test` d15 vector and `plus_p8_test` expectation changes, and show d15
   fails on master before the RTL change.
2. Re-derive every changed expectation on paper with source citations (Arnold V section 2.6,
   the S22 PPR diagram). Never copy numbers from the simulator or from `190f4d3`.
3. Re-run the Sonic counterfactual with `docs/screenshots/sonic-cartwait-2026-09-22/`
   (`prepare.py`, `run.sh`, `analyze.py`; ignored, in the main checkout) against the branch's
   real RTL, and confirm the 312-line recurrence.
4. Gate `python3 sim/select_tests.py --run` plus `make -C sim/plus d5-cart-timing`, then a
   cross-provider review (or a review-debt row).

Device acceptance (last step; automated through the `mister-capture` skill):

- **Ownership first.** Use the MiSTer only when no other session owns it (check with the user
  or the device-acceptance session's handoff). If it is busy, stop at READY with this plan.
- **Builds.** A = latest full-effort master RBF that contains the cartridge fix (`03f4724` or
  later). B = the branch, synthesised by an explicitly requested pre-merge build per
  `docs/ci-testing-policy.md`. Record both RBF SHA-256 values on the device.
- **Setup.** Original Sonic CPR, SHA-256
  `4cb31c7f1769989a029fe1bd7ffb04670d5d24570dc56b0417fac662fed887ae`; CFG model 6128 Plus
  (`[34:33]=2`), Sync Full (`[36:35]=0`); save and restore the original CFG and MENU exactly as
  the [rearm-boundary runs](investigations/sonic/rearm-boundary-2026-09-22.md) did.
- **Sonic controls, both builds.** No-input control first (title checkpoint on the 18-second
  boot recipe, then captures 8 and 16 seconds later), then the sustained-fire control (two-second
  fire hold, captures 2, 8 and 16 seconds after release). Inspect every native PNG for the scene
  it actually shows; elapsed time is not a checkpoint.
- **Pass for B:** coherent title without displaced bands, and progression to the Act 1 card and
  playfield at least as reliably as A. A gameplay band that remains is recorded, not a failure
  by itself.
- **Regression set, both builds:** Copter 271 (title flash, gameplay scrolling), Burnin' Rubber,
  Pang, Plotting, Navy Seals, the CRTC3 demo. An A-versus-B difference belongs to the PAUSE
  rule; a problem already present in A is triaged under B21.
- **Outcome.** B passes and nothing regresses: mark READY for integration and close B20-7's
  Sonic part. B still fails while A is no worse: record that the rule is wrong for hardware in
  B20-7 and fall back to the short-list discriminators. Keep the general PAUSE 0/1, PPR and
  REPEAT boundaries open either way.
...
**DEVICE ACCEPTANCE COMPLETE (PHASE 1), 2026-09-13:** Phase 1 (the format-1 event ring
in `rtl/ssm_marker.v` and `csl_runner.py`) was verified on real hardware: Module A executed
...
capture fixture and automated round trip (acceptance 2 and 3) are implemented and
independently reviewed as one production-T80 fixture (2026-09-22): six classic model/CRTC
cases check header bytes, RAM, restored loader-consumed state and host publication races.
Physical memory arbitration, dynamic FDC/video and SNA-unrepresented state remain
outside that fixture's scope. See the [design](b18-sna-save.md).
...
computer over SSH. Saving straight to SD needs either a Main_MiSTer change, to be discussed
upstream, or Main's existing save-state copy. The latter would move the disk drives off SD slot
0, produce `.ss` files instead of `.sna`, and require changing the slot header to Main's 32-bit
format. The design's "Status and limitation" section has the details. Main's `ioctl_upload` path serves C64/C128 only. Main's
