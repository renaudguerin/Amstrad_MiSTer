...
### 17.1 VMA/VMA' pointer model (§17.1, p.177)
...
### 17.2 R1 vs R0 relationship (§17.2, p.178-182)
...
- p.184 render-verified 2026-08-24: the worked example (display width 40 chars, "&28") confirms
  ...
  itself is plain text at p.183 bottom and needed no visual tier.

### 17.4 VMA'/VMA when C4=0 (§17.4, p.183-185) — KEY TYPE-0 vs TYPE-1 DIVERGENCE
...
- **R1 update timing at the exact JIT instant** (p.183 bottom): if R1 is rewritten *exactly* when `C0=R1` (before/during that comparator evaluation), the compare is missed (VMA' not updated that pass). If rewritten one cycle later when `C0=R1+1` (i.e. C0 already past old R1), the *new* comparison condition (`C0=R1` with new R1 value) can still be satisfied and updates VMA'. p.184 render-verified 2026-08-24: worked example uses R1=40 (&28)→255 with the C4=24/25 boundary; condense to: "write ordering relative to the comparator's evaluation edge determines whether that pass's VMA' latch fires."

*(CRTC2/3/4 for contrast only: type 2 has a documented priority-ordering bug between the "last line of frame" test and the C0=R1 test when R1=0, involving a partial AND-only update bug on VMA'. Out of scope for type 0/1 modeling — see p.184-185 if ever needed.)*

### 17.5 Acknowledgment of R1=0 (§17.5, p.186) — timing/deadline table
- **Type 0 (and type 1, type 2 grouped together in the doc):** writing `R1,0` must land by the
  **first character (C0=0) of the target line** to be honored for that line. p.186 render-verified
...
- **CRTC3/4 contrast:** their acknowledgment window closes **two** characters earlier — writes
  through C0=3e are just in time; the first too-late write is C0=3f (p.186 §17.5.2, render-
...
### 17.6 Interline border (§17.6, p.186-188)
- **R1=R0 and C0=R0, all CRTC types (0 through 4):** exactly 1 µs (one character) of border is generated on the last character (`C0=R0`) even though R1=R0 — i.e. **the very last displayed character position always shows border for that cycle**, because the DISPTMG-off condition `C0=R1` and `C0=R0`'s natural wraparound coincide, forcing a 1-char border blip before the next row's `C0=0` reload. VRAM pointer offset continues normally into the next character-row's start (§17.6.1, p.186-187).
- **R1>R0 and C0=R0 — type 0 (and type 2 for contrast):**
  - Border activation is **anticipated by 0.5 µs**: type 0 raises "BORDER ON" to the Gate Array as soon as `C0=R0` is reached (since `C0=R1` will never be reached, R1>R0), i.e. **one byte of border (0.5 µs) is generated between character rows** even though R1 was programmed to allow more display width (§17.6.2, p.187).
  ...
  - Type 0 can **suppress this spurious border byte** via the R8 SKEW-DISPTMG delay functions (§19.2, p.194) — cf. type 2 which has no SKEW-DISPTMG and therefore cannot suppress it.
  - VRAM pointer gets "stuck" at VMA' in this configuration unless R12/R13 are used to force a reload (Note 2, p.187).
- **R1>R0 and C0=R0 — type 1 (grouped with 3/4 for contrast):** **no border byte is generated between rows at all** — rows can be seamlessly contiguous in the displayable area (used for "frame merging"/plagiarism effects per the text, p.187-188).
...
### 18.2 R6 deadlines and priorities (§18.2, p.189-190)
...
### 18.3 R6 conflicts (§18.3, p.190-192)

**General (§18.3.1, p.190-191):** R1=0 has an analogous but resolved conflict (deactivation `C0=0`/R0-wrap wins over activation `C0=R1=0`) — deactivation wins. R6's conflict resolution is much less clean.

**Type 0 conflict at C4=R6=0, C9=0 (§18.3.2, p.191):**
...
- If R6 is subsequently updated to a value >0, the alternation stops — border does **not**
  ...
  `C0=R1` only if **R6 is still 0 then** (v1.11 FR §18.3.2 p.191). English p.191 changes this
...
**Type 1 conflict at R6=0 (§18.3.3, p.192):**
...
### 19.2 SKEW-DISPTMG functions — types 0/3/4 (§19.2, p.194-198)
...
- **BORDER OFF** (`R8 = 000000xx`, §19.2.2, p.194): stops managing the other SKEW sub-functions (returns to normal C0=R1/C0=R0 comparator-driven border).
- **BORDER DELAY +1 / +2** (`R8 = 000100xx` / `001000xx`, §19.2.3, p.194-195): delays the R1-border transitions by 1 or 2 CRTC characters:
  ...
  - The VMA'→VMA row-end/row-start latch timing (on `C9=R9`/`C0=R1`) is **unaffected** by this delay — only the visible border edges shift, not the pointer bookkeeping (§19.2.3, p.195).
  ...
  - **R8 changes affecting this delay take effect immediately, mid-line** (§19.2.3, p.195) — worked diagram at p.195 (R1=59,R0=63) shown but condensed here: writing R8 mid-line changes where the border edge appears on the *same* line, live.
- **No-condition case R1>R0** (§19.2.4, p.196): since `C0=R1` never fires, type 0 (and type 2) substitute `C0=R0` as the effective border-start trigger (spurious border byte, per §17.6.2). **Type 1 (and 3/4)** do not substitute anything — no BORDER-ON signal is sent at all in this configuration. When a SKEW-DISPTMG delay is programmed on type 0 in this R1>R0 configuration, the delay is still counted from C0 transitions as if the (never-reached) `C0=R1` condition had fired at the substituted point.
- **Disintegration of the border via double R8 write (type 0 only)** (§19.2.5, p.197-198): a 1-2 µs window exists (using SKEW-DISPTMG) during which **two back-to-back R8 writes on the same/adjacent line can cancel a spurious border byte** entirely — type 2 cannot do this (no SKEW-DISPTMG). Four documented sub-cases (§19.2.5.1-4, p.197-198) depending on exactly which C0 value the two OUTs land on relative to R0/R1 — condensed: **whether the border byte appears or is cancelled depends on which of the two R8 writes is "seen" by the comparator logic in the cycle it fires**, i.e. a last-write-wins-if-in-time race. The four sub-case *rules* are plain prose and readable (flag narrowed per review B7); ⚠ keep it only for the p.197-198 cycle diagrams — each shows a 1-cycle boundary case that's easy to mis-transcribe from the compressed text.

### 19.3 Interlace functions — general (§19.3, p.199-203)
...
**Type 1 parity states (§19.5.3, p.209-210):**
...
- **Key type-1 vs type-0 divergence:** type 1 has **no VSYNC delay-by-1-line correction** for odd C4s — positioning R7 on an odd C4 when R9 is even **will** create a permanent 1-line VSYNC gap between even/odd frames (no self-correction mechanism) (worked table p.209, "EVEN FRAME"/"ODD FRAME" with `+32` markers).
- **R8 toggle timing is cycle-precise and documented exactly** (§19.5.3, p.210) — this is implementation-critical:
  ...
  - ⚠ VERIFY p.211-213 → narrowed to pp.211-212 (render-verified 2026-08-24; p.213 is §19.5.4
...
**Type 0 (grouped with 1, 2 in §19.7.2, p.219):**
...
### 19.8 Counting in Interlace Videomode — per-type C9 arithmetic (§19.8, p.220-235)

**Type 0 (§19.8.1, p.220-225):**
...
- End-of-character-row test, from the line **following** the one where R8 went to 3:
  ```
  if ((C9*2) | ParityFrame) == (R9 + ParityFrame):     // bitwise OR on both sides ("R9 or ParityFrame", p.221) — the source writes '+' loosely
      if C4 == R4:
          C4 = 0
...
- **On and after the line where R8 returns to 0** (leaving IVM): the test compares the last
  ...
  lines even while live C9 advances past R9; p.221's recovery recipe programs R9 to the frozen
  value. This asymmetry is the single most bug-prone edge in type-0 IVM and directly affects
  the `VMA'←VMA` row-end latch too (§19.8.1, p.220-221; Q19(b), resolved visually 2026-08-26).
- p.222-225 render-verified 2026-08-24. Entry tables switch R8→3 at C9 offsets 0,1,2,3 (p.222),
  4,5 (p.223) and 6 (p.224 top); exit tables switch R8→0 at C9 offsets 0-3 (p.224 bottom,
  p.225). **All of them use R9=6 (even), stated in prose on p.221**; an earlier reading of them
  as R9=3 produced three spurious contradictions, all now withdrawn:
  - *Parity.* With R9 even, §19.5.2 (p.206) states outright that "the parity is identical
  ...
  - *Row end.* Reading the compared value as bitwise OR — p.221 spells it "R9 or ParityFrame
  ...
  - *Overflow.* Switches at C9=3,4,5 overflow in both parities (C9 runs to 19, C9.VMA back to
    ...
    target. The switch at C9=6=R9 discriminates: p.224 top shows the even frame resetting
    (next row C4=1, C9=0) and the odd frame overflowing (C9 7..19), matching p.220's
    "If C9=R9 and the parity is odd… C9 is not reset to 0 and overflows".
  - *Exit.* p.224 bottom-right (even frame, R8→0 on the line C9=3 / C9.VMA=6=R9) resets and
    increments C4; p.225 bottom-right (odd frame, C9=3 but C9.VMA=7=R9+1) does not — C9 goes
    to 4 inside the same character. That confirms both the parity-dropped `C9.VMA == R9` exit
    test and the p.208/p.221 "IVM disabled on C9.VMA=R9+1" counting bug.
  Adjudicated conflicts and remaining table anomaly (Q19):
  - the p.220 gate `If R9.0=0` is a source typo for R9 **odd**; p.206, the p.207 R9=7
    example, p.209's type-1 contrast, and the R9=6 tables all agree. Odd-R9 behavior is F15.
  - p.221 resolves the compared-value phases: switch line uses R9-or-ParityFrame, steady IVM
  ...
  - the seven non-matches all run through C9=7 and the frozen-6 control resets. One non-match,
    p.224 bottom-left, also shows C4=2 (an increment without C9 reset); exclude only that C4
    cell from fixtures pending hardware/author confirmation. Post-exit RTL is F16.
  - in the settled C4≥1 blocks of p.223 (both frames) and p.224-top even frame, the C9 column
    prints doubled values inconsistent with the C9-VMA beside it (p.222 and p.224-top odd print
...
### 20.1-20.2 General & calculation (§20.1-20.2, p.242)
...
### 20.3 Update conditions per type (§20.3, p.243-244)
...
- **Known real-world consequence** (p.243, explicit game reference): because type 1 keeps applying `VMA=R12/R13` throughout the entire C4=0 character row (not just the very first scanline), software written for/tested against type 0 that changes R12/R13 "too early" while still nominally in row C4=0 can have that new address **prematurely override** the row it was meant for, on type 1 only. Cited bug: *007 The Living Daylights* (Domark, 1987) — vegetation/decor corruption from a score-zone address write landing during C4=0 on a type-1 machine. **This is a direct behavioral divergence a naive model must reproduce**: type 0 only re-applies R12/R13 at the single instant `C4=C9=C0=0`; type 1 re-applies it (to VMA, immediately) on **every** write that occurs anywhere within `C4=0` (any C9, any C0=0 boundary) — see worked timing diagram p.243 (`OUT R12,#30` mid-row, offset changes on the very next character despite still being within C4=0 on type 1, whereas the analogous type-0 diagram would only pick it up at exact `C4=C9=C0=0`).
...
### 21.2 Reading register contents (§21.2, p.246-247)

Register select port: `&BC00` (both types). Register data port: `&BF00` (both types, for read).

**Type 0 (§21.2.1, p.246):** only 5 LSBs of the register-select number are decoded (upper 3 bits ignored ⇒ aliasing every 32). Readable registers at `&BF00`:
...
**Type 1 (§21.2.2, p.246-247):** also 5-LSB register decode. Readable registers at `&BF00`:
...
### 21.3 Reading status (§21.3, p.247-249)

**General (§21.3.1, p.247):** Only type 1 has a genuine status register, at dedicated port `&BE00`.
...
- Also: if R6 is set to 0 while C4>0 specifically to *force* border (the "R6=0 special case" from §18.2.3/18.3.3), that state is **not reflected** in status bit 5 — bit 5 stays 0 in that case (since the underlying `C4=R6` equivalence, which is what bit 5 actually tracks, is not what's driving the border here). The bit-5 transition rules are fully stated in prose (flag narrowed per review B7); ⚠ keep it only for the two worked timing diagrams (p.248) showing bit 5's transition exactly at the C0=R0 sample point across a VSYNC boundary — re-derive against C0=R0 sampling in the Verilog rather than trusting the ASCII diagram positions verbatim when pin-exact vectors are needed (diagrams are on p.248 only — render-verified 2026-08-24).

*(Type 3/4 status registers R10/R11 documented in detail at §21.3.4, p.249-250 — out of scope for type 0/1 modeling; noted only because §21.3.1 states the &BE00 port mirrors &BF00 on those types, unlike type 0's floating/type 1's dedicated-status behavior.)*

### 21.4 Dummy register 31 (§21.4, p.250)
...
(§22, p.251 — brief chapter, not much CRTC-type-specific behavior beyond what's already covered in §21.)
...
### 28.1.1 Via C4/C9 overflow (§28.1.1, p.293)
...
### 28.1.3 Via VSYNC activation timing when C0>0 (§28.1.3, p.293)
...
### 28.1.4 Via VSYNC length (§28.1.4, p.293)
...
### 28.1.5 Via HSYNC length with R3=0 (§28.1.5, p.293)
...
### 28.1.6 Via border, visually (§28.1.6, p.294)
...
### 28.1.7 Via interlace mode C4-counting rate (§28.1.7, p.294)
...
### 28.1.9 Via register read at &BF00 — PRIMARY 0-vs-1 TEST, register-set comparison (§28.1.9, p.294)
...
## 29. CPC identification (§29, p.295)
...
### 29.1.1 Extended-features unlock (§29.1.1, p.295)
...
### 29.1.2 PPI Port C bug (§29.1.2, p.295)
...
### 29.1.3 PPI Port B bug (§29.1.3, p.295)
...
6. **Dynamic R1 rewrite timing at the exact `C0=R1` instant** is a genuine race (write lands before vs. after the comparator's evaluation edge) that changes whether the VMA'←VMA row-end latch fires that pass (§17.3/17.4, p.183-184) — easy to get "off by one cycle" in RTL.
...
the p.184 worked example (initial R1=40/&28).
...
Retired by the 2026-08-24 D1 visual re-verification: p.184 (JIT worked table confirmed; example
uses R1=40/&28, not 64), p.186 (deadline boundary corrected — see §17.5), p.197-198 (all four
§19.2.5.x sub-case diagrams match), p.207-208 (odd-R9 parity/VSYNC-delay tables match),
p.211-212 (16 type-1 parity truth tables match; p.213 is §19.5.4 CRTC 2, out of scope),
p.248 (bit-5 transition diagrams match; p.249 is §21.3.4 CRTC 3/4 STATUS 1, out of scope).

Resolved source conflicts / remaining table anomaly:

- p.220 — the IVM parity-gate token `If R9.0=0` is a one-bit source typo for odd R9 (Q19,
