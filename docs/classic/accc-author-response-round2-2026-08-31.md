...
| §16.4.1.2 English p.169 | The two English-only R0/VSYNC paragraphs are normative and were intended to be added to French; their absence followed an update problem around §13.2.2. | Remove the “supplemental/unconfirmed” quarantine, but retain the English v1.11 page as the published anchor until a future French edition exists. The 2026-09-01 consequence audit found and corrected the steady-R0=1 preceding-line qualification gap with failure-first `t02l`; `t02m`/`t02n` pin the exact dynamic writes. `t02o` pins blocked-comparison consumption only as a model inference and hardware discriminator, not as an author-confirmed consequence. See `archive/accc-author-response-consequence-audit-2026-09-01.md`. |
