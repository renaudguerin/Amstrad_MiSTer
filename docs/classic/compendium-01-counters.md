...
## 1. Building a frame — general logic (Ch.6, p.34-35)

- Idealized skeleton, NOT the real per-CRTC model (§6.1.1, p.34): C0++ each cycle; at C0==R0,
...
- HSYNC starts at C0==R2 (C3l=0, incr while C3l<R3l, ends at C3l==R3l). VSYNC starts at C4==R7
  (C3h=0, incr, ends at C3h==R3h, or ==16 if the field doesn't apply) (§6.1.2, p.34).
- DISPEN horizontal: enabled at C0==0, disabled at C0==R1. DISPEN vertical: enabled at C4==0,
  disabled at C4==R6 (§6.1.3, p.34).
- Precise video-pointer algorithm (§6.1.4, p.35):
  ```
  If C0==R0: C0=0
    If C9==R9: C9=0
  ...
  Else: C0=C0+1
  ```
- C9 occupies VRAM address bits 11-13 (p.35 table confirmed from the text layer; earlier
...
## 2. Synchronization principles (Ch.7, p.38-42)
...
- All timing in this book is referenced to **C0vs**: the instant C0 is considered 0 from the
  ...
  GA") (§7.1, p.38-39). For CRTC 0/1/2, **GA's HSYNC generation uses the CRTC's own
...
- VSYNC pin raised when C4==R7 — the single reliable sync point (§7.2, p.39). PPI port B bit 0
  mirrors it with no extra CRTC-side delay. Standard frame period: 312×64µs = 19968µs.
- Fake VSYNC (§7.3, p.42): PPI-wiring trick, out of scope for a CRTC-only model.

## 3. Register R9 — scanline counter C9 (Ch.10, p.74-80)

- R9 is 5 bits (0-31); only bits 0-2 feed the VRAM address ("curtain" wrap past C9=7), but full
  5-bit C9/R9 values are used for counting comparisons (§10.1, p.74).
- **R9 update is considered while C0<=R0** — live comparator input, honoured for the rollover
  at the end of that same character line (§10.2, p.75). p.75 chronograms render-verified
...
- **The CRTC does not buffer C9/C4 against R9/R4** — it re-evaluates the raw equality each
  cycle; only VMA' is actually latched (at C0==R1) (§10.3.1, p.76). If R9 is rewritten to 0
  while C9>0, C9 does **not** snap to 0 — it overflows up to 31 then wraps, unless the
  last-line exception applies (§10.3, p.75).

### 3.1 CRTC 0 — last-line and adjustment arbitration (§10.3.1, p.76-77; §12.2, pp.93-95, CRITICAL)

- CRTC 0 compares C9/R9 and C4/R4 while **C0<2** to establish `Last Line`.
- **C0<2 evaluation timing (ACCC v1.11 §12.2, p.93)**: Modifying R4 or R9 on C0<2 immediately
  evaluates the updated values to validate or clear the `Last Line` state (so anticipating the
  programming on a prior line is unnecessary). (Note: on CRTC 2, §12.4.1 p.96 specifies that
...
- With `Last Line` false, the live general case applies at rollover: C9==R9 increments C4;
  ...
  (§10.3.1.1, p.76).
- **⚠ Dynamic-update hazard — R9 written exactly at C0==R0** (p.77): C4 may still increment
  ...
  p.83 example 3, see §4.2. Both cases belong in t12.
...
### 3.2 CRTC 1 — "pure logic", no latch window (§10.3.2, p.77)
...
- **No last-line exception at all** — comparisons are always live, at every C0 (§10.3.2.2,
  p.77): "pure logic... unspeakable and tasteless simplicity." This is the structural
  difference from CRTC 0 a shared hcc/line/row model must special-case.

### 3.3 CRTC 0/1 divergence cross-check (§10.3, p.79-80 tables)

Cross-check tables (p.79-80) read cleanly in the text layer; the earlier heavy-smearing flag
...
## 4. Register R5 — vertical adjustment counter (Ch.11, p.81-92)
...
- **CRTC 0 has no separate C5 — reuses C9**, comparing C9 vs R5 instead of R9 once in
  ...
  C9 keeps counting "characters" within each using R9) (§11.1, p.81-82).
...
### 4.1 Worked example (R4=10, R5=16, R9=3, R1=40, R0=63) (§11.2.1, p.82)
...
- CRTC 0: C4 freezes at 11 (R4+1) for the whole adjustment; C9 counts 0..15 continuously;
  ...
  (p.82 table, render-verified 2026-08-24; corrects this digest's earlier "4 distinct
  addresses" misread).
- RTL mapping (adjudicated 2026-08-25 against the renders and the code; vectors t25a-t25c):
  the LINE column is the C9-driven segment of the **composed** address — §20.2 p.242 takes
  ...
  character inside adjustment lines: the PTR-VRAM column plus the p.84 prose ("updated with
...
### 4.2 CRTC 0 adjustment detail (§11.2.2, p.82-84)
...
- Writes on that entry line change the selected comparison:
  ...
  - Companion case (p.83, example 3): an R9 write landing inside the same `C0∈[2,R0−1]` window
...
### 4.3 CRTC 1 adjustment detail (§11.2.3-11.2.4, p.84-85)
...
- **⚠ VMA source switches during first adjustment line** (IMPORTANT, §11.2.4, p.84-85): if C4
  ...
  VMA is NOT updated from R12/R13 at C4==1. Conversely (§11.2.4 note, p.85): an **R9** write
...
### 4.4 R5 updated mid-adjustment (§11.3, EN pp.86–87; FR pp.87–88)
...
- CRTC 0 (§11.3.1, p.86): reaching R5 stops adjustment; R5 rewritten to LESS than C9+1 →
...
- CRTC 1 (§11.3.2, p.86): same "reaches R5 stops" logic on `C5+1`; less-than → C5 overflows.
  ...
  - **C4/R4 reset confirmed by the author (Q20/N2, response 2026-08-31):** the specific
    ...
    whereas §11.2.4 FR p.85 (EN p.85) generally describes adjustment increments ignoring
...
### 4.5 R5 update BEFORE adjustment starts (§11.4, p.87)
...
- CRTC 0 (§11.4.2, p.87): R5>0 update arriving **after C0>2** on the last frame line is not
...
### 4.6 VSYNC during vertical adjustment (§11.5, p.87)
...
## 5. "Rupture For Dummies" (RFD) — CRTC 1 only (Ch.11.6, p.88-91)
...
- **Trigger**: writing R5 **non-zero**, at **C0==R0**, on a line where R5 was previously **0**.
  ...
  interaction: §8.7.
- **Two independent flags activated** (§11.6, restated §11.6.2 p.89-90):
...
- **Frame-parity alternation** (§11.6.1, p.89-90): once armed, C9==R9-at-C0==R1 becomes
  parity-dependent:
...
- **"IVM ON/OFF" freezes parity** (§11.6.2, p.89-90): `OUT R8,3` then `OUT R8,0`, timed on an
...
- **RFD#10, CRTC "1-B" only** (§11.6.2, p.90, ⚠ unverified hardware taxonomy — no reliable way
...
- **Recipe** (§11.6.3, p.91): `OUT R5,1` then `OUT R5,0`, with the R5,1 write landing exactly
  at C0==R0 on a line where C9!=R9 for the target C4. Modify R12/R13 the line *before* the R5
  update takes effect.
- **CRTC 3/4 do not have RFD.** HITACHI's CRTC-0 technical guide (p.121 anomaly table) is cited
  ...
  playground!" (p.91, render-verified 2026-08-24; the HITACHI table is not reproduced in the
  compendium). Do not port RFD to CRTC 0 without independent verification.

## 6. R6 and vertical adjustment; interlace interactions (§11.7-11.9, p.91-92)
...
- **CRTC 1 special R6==0 case** ("split-border"): BORDER activation via C4==R6 is handled
  ...
  2026-08-24: p.91 asserts the mechanism in prose without spelling it out and shows no figure;
  cross-ref Ch.19.2 confirmed to start at p.194).
...
## 7. Register R4 — character row counter C4 (Ch.12, p.93-102)
...
- VMA reload from R12/R13 fires when C4 transitions to 0, and for CRTC 1 specifically "while it
  ...
  transition instant (§12.1, p.93) — general form of the §4.3 adjustment special case.

### 7.1 CRTC 0 (§12.2, p.93-95)
...
- **RLAL case study, first-line variant** (§12.2.1, p.94): setting R9=R4=0 after the C0<2
...
- **RLAL from the genuine last line** (v1.11 FR §12.2.1 p.96; EN p.95): the two write windows
...
## 8. Register R0 — horizontal counter C0 (Ch.13, p.103-130)
...
### 8.1 CRTC 0 — the "first 3 microseconds" state machine (§13.2.1, p.104-105, CRITICAL)
...
- **Consequences of R0==0** (stated in the §13.2.1 run-on, p.105; restated under §13.2.4,
  p.106): C0 never advances past 0, so the
  ...
  - C9!=R9 at the (only) C0==0 when R0 became 0 → **all counters freeze** entirely while
    ...
    (p.105), i.e. an N-µs stall costs N/64 lines. The "(C4−1 if R9=7)" gloss converts those
    eight lines to one C4 character-row period; it does not decrement C4 (author-question Q2,
    resolved 2026-08-26 against the p.109 freeze case study).
  ...
  - Worked test vectors (§13.2.6, p.109-110), all regs C0=R0=C4=R4=C9=R9=R5=0: 1st C0==0 → VMA
...
- **Case R0==1** (§13.2.1 pp.104-105; §13.2.5 p.108): C0 alternates 0,1,0,1... (2µs "lines"), never
  ...
  of 2 µsec before ceasing", p.105; alternating pattern in the p.108 case study). The chained
  into-another-adjustment-frame reading applies only while R5>0. No figures exist in
  §13.2.1-13.2.6 (pp.104-109 render-verified 2026-08-24): the alternation is prose-only, and
  the sole figure (the p.109 R0=0 worked table) extracted cleanly and matches the render.

### 8.2 CRTC 0 — VSYNC freeze window (§13.2.2, p.106)
...
### 8.3 CRTC 0 — additional-line freeze under R0==0 (§13.2.3, p.106)
...
### 8.4 CRTC 0 — R0==1 overflow trap (§13.7.2, p.125-127)

- General (v1.11 FR §13.7.2 pp.126–128; EN pp.125–127): at **C0==0**, the CRTC programs the
...
### 8.5 CRTC 1 — R0 accepts any value freely (§13.3, p.114)
...
- **⚠ Instruction-dependent write-timing hazard — OUTI vs OUT(C),reg8** (§13.3/§13.7.1.1,
  p.114/125): the µs at which R0's new value reaches the C0==R0 comparator depends on which Z80
  ...
  - **§13.6 R0 UPDATE, p.123-124** — §13.6.1 "CRTC 0, 2 : CHRONOGRAM" and §13.6.2 "CRTC 1 :
    CHRONOGRAM", both p.123; §13.6.3 covers CRTC 3/4, p.124.
...
### 8.6 CRTC 1 — R0-update-driven RFD trigger (§13.7.1.2, p.125)
...
### 8.8 Offset (R12/R13) latch timing vs C0 (§13.8, p.127-130)
...
- **R0==3 (4µs "frames")** (§13.8.1, p.128): CRTC 0 and CRTC 1 behave identically — `OUT R13,4`
  is picked up starting the VRAM-address cycle immediately following the write, effective once
  C4 returns to/stays at 0 (R4=0 here so every row is row 0, fires every 4µs frame). The p.128
  grids render clean at 200 dpi (render-verified 2026-08-24; see the note at the end of this
  section).
- **R0==1 (2µs "frames")** (§13.8.2, p.129, IMPORTANT): explicit caption "the event C0==R0
...
- **R0==0 (1µs "frames")** (§13.8.3, p.130, IMPORTANT): explicit caption "R12/R13 cannot be
...
- Grids render-verified 2026-08-24 (pp.128-130): legible at 200 dpi; captions confirmed
  verbatim on p.129 ("The event C0 = R0 after 2 µsec leaves C4 = 1 for the 2nd period of
  2 µsec, which represents a vertical adjustment 'not canceled' (because C0 is never equal
  to 2)") and p.130 ("R12 / R13 cannot be considered until C4 and C9 both go back to 0");
...
- R.V.I./R.V.L.L. programming-technique lookup tables (§13.3.1, §13.4.1, pp.114-121) — demo
