...
1. **p.76-77 (§10.3.1) — last-line state and C0>1 writes.** The text says a last-line state
   ...
   pp.93-94 say that once the state is armed, mid-line writes at C0>1 do not change the
   ...
   **RESOLVED BY DEFAULT READING 2026-08-26** (fresh pass pp.76-77, 83, 93-94; no
   author answer). There is **no late arming**. §12.2 p.93 says that R4/R9 must establish the
   equality while C0<2 to validate `Last Line`; §12.2.1 p.94 then demonstrates the converse:
   setting R4=R9=0 after that window on the first C4=C9=0 line does not make it last, and C4
   becomes 1 on the following line. The p.76 sentence therefore describes an already-armed
   state surviving C0>1 writes when adjustment was not selected at C0=2, not a second arming
   window. C0>1 writes can still affect the live adjustment/line-end paths documented on p.83,
   but cannot newly validate the latch. The existing F12 windowed-latch reading stands.
2. **p.105 (in the §13.2.1 run-on; rule restated under §13.2.4, p.106) — cost of the R0=0
   freeze.** The p.105 prose says freezing R0=0 for
   ...
   **RESOLVED BY DEFAULT READING 2026-08-26.** This is elapsed-time accounting, not a
   ...
   The detailed p.109 case study confirms that C9 is frozen, C4 may perform only the separately
...
3. **p.108 (§13.2.5) — ending the R0=1 adjustment with R5=0.** The case study states "for some
   ...
   **BEHAVIOR CONFIRMED; CAUSE STILL UNSOURCED 2026-08-26.** The rendered p.108 still says
   explicitly that the author had not determined the reason. The adjacent p.109 R0=0 exit
   example exercises the normal `C9+1` form ("C9+1 being different from R5, then C9 is
   incremented"), so the p.108 `C9` comparison is not merely loose notation for the general
...
4. **p.89 (§11.6.1) — repeated-RFD sentence.** "A RFD triggered on the last line C9=R9
...
5. **p.131 (§14.1) — HSYNC start vs stop.** After describing C3l counting from C0=R2 until
...
6. **p.134 (§14.4) — C-HSYNC duration table cells.** Please confirm each cell gives the
...
7. **p.147 (§15.1) — deflector-lock threshold.** "If the HSYNC is too short (>2 µsec and
   ...
   **RESOLVED 2026-08-26: the inequalities are correct.** The rendered parenthetical decodes
   ...
   wrong.
8. **p.168 (§16.3) — mechanism 2 on CRTC 2.** Mechanism 2 is stated present on CRTCs 0/1 and
...
9. **p.194 (§19.2.1) — BORDER ON pointer update condition.** BORDER ON states the current
   ...
   **RESOLVED BY DEFAULT READING 2026-08-26.** It is a garbled restatement of the normal
   ...
   impossible unless R1=0. §17.1 p.177 gives the exact row-end rule in the same sentence
...
10. **pp.199/206/217 (§19.3/§19.5.1/§19.6.1) — which frame gets the extra interlace line.**
    §19.3 says it ends "the first frame"; §19.5.1/§19.6.1 attach it to completion of the
    **even** frame's construction; p.200 shows the odd frame lasting 20032µs "inheriting" it.
    Which frame receives the additional line, and is the odd/even labelling relative to
    MID-VSYNC frames or to ParityFrame?

    **RESOLVED BY DEFAULT READING 2026-08-25** (fresh render pass pp.199-200, 206, 207, 217;
    ...
    - §19.6.2 p.217 (render): type 1 adds the line "at the end of the frame (after the R5
      lines if necessary)" iff R8∈{1,3} and **ParityFrame is even**; §19.6.1 p.217: type 0
    ...
    - §19.5.1 p.206 (render): the same even frame carries both events — extra line "when the
      ...
      p.207 table's column headers (PARITYFRAME=ODD / PARITYFRAME=EVEN) confirm the vocabulary.
    - §19.3 p.200 (render): "1 even frame with a duration of 19968µsec… 1 odd frame, which
    ...
    - Counter accounting: §19.6.1 — type 0 increments C4 once for all additional lines
      ...
      when R9+1 is a multiple of R5. Matches §11.2.4 p.85 ("added only at the end of a even
      frame").
...
11. **p.206 (§19.5.2) — even R9 total line count.** IVM programming with even R9 yields an
    ...
    **RESOLVED BY DEFAULT READING 2026-08-25.** The p.206 render states it directly: "In IVM
...
12. **§19.5.2 — odd-C4 IVM activation imbalance; English qualifier omitted.**
    ...
    **RESOLVED BY EDITION COMPARISON 2026-08-28.** French v1.11 p.208 specifies that
    R8 goes to 3 on C4=1, C9=0 “à chaque frame” (on every frame). English p.207 omits
    ...
    ParityC9 is recalculated from C4.0 xor ParityFrame (§19.5.2 English pp.206–207;
    §19.8.1 FR p.220, EN p.220). This does not undo an elapsed timing offset. ParityR6
    persists and
...
13. **p.246 vs p.294 (§21.2.2 vs §28.1.9) — type-1 readable registers.** §21.2.2 documents
...
14. **p.294 vs pp.247-249 (§28.1.8 vs §21.3.3) — status bit numbering.** The identification
    test polls "the transition of **bit 6**", but §21.3.3's frame-timed examples transition
    bit 5, while the CRTC 3/4 STATUS-1 table (p.249) has a separate always-1 bit 6. Which bit
    ...
    **RESOLVED VISUALLY + CHIP DOCUMENTATION 2026-08-26.** The frame-timed identification bit
    is **bit 5**. The p.247 render and UM6845R Figure 3 both map bit 6 to `L` (light-pen
    ...
    6 on p.249 belongs only to CRTCs 3/4 and is a separate always-1 field. The RTL's bit-5-only
    status view is correct for a CPC with no light-pen event source.

15. **p.191 — border-alternation condition wording — RESOLVED BY DEFAULT READING
    ...
    blocked.
16. **p.189 (§18.1) — "(1st line-character R6)" — RESOLVED BY DEFAULT READING
...
17. **p.293 vs pp.87-90 (§28.1.1 vs §§11.2/11.4) — type-1 VSYNC discriminator boundary.**
...
18. **§13.6.2 p.123 vs §13.7.1.2 p.125 — R0-widening RFD arming condition.**
    ...
    **RESOLVED BY DEFAULT READING 2026-08-26.** Arming is end-state based. The p.123 render
...
19. **p.220 (§19.8.1) — one-bit polarity of the type-0 IVM parity gate.** In the C9 reset
    ...
    **even**; the gloss, §19.5.2 (p.206: with an even R9 "the parity is identical regardless of
    the value of C4"; with an odd R9 the line parity "depends on that of C4 and on the current
    parity at the start of the frame"), the p.207 worked R9=7 example, §19.5.3's mirror-image
    statement for CRTC 1, and the pp.222-225 tables themselves (R9=6, no per-C4 alternation in
    ...
    written "R9 or ParityFrame" on p.220 but "R9 or ParityC9" on p.221 — identical while R9 is
    even, different exactly in the alternating odd-R9 case; which one is the hardware
    comparison? (b) in the exit tables (p.224 bottom, p.225) the character keeps counting to
    ...
    this question rested on misreading pp.222-225 as R9=3 and is withdrawn.)

    **Main token + (a): RESOLVED BY DEFAULT READING 2026-08-25** (fresh render pass pp.220,
    220, 223, 224; no author answer). `If R9.0=0` is a typo for `R9.0=1` — the row-end
    ParityC9 update fires only when R9 is **odd**. The gloss, §19.5.2 p.206 (even R9 → parity
    identical regardless of C4, so no per-C4 update), the pp.222-225 R9=6 tables (no
    alternation in any panel), and the p.207 R9=7 example (per-C4 alternation) are unanimous;
    the literal token would make every one of them wrong. (a) is settled by the p.221 prose,
    ...
    **exit/write line** tests C9.VMA against plain R9 ("ParityC9 is no longer considered for
    R9"). The p.220 pseudocode line `((C9 x 2) or ParityFrame) == (R9 + ParityFrame)` is the
    ...
    out. The p.221 worked recipe, which programs R9 to the frozen C9.VMA value to recover the
    reset, independently agrees. One p.224 bottom-left cell remains anomalous: it ends at
...
20. **§11.3.2 — C4 comparison during stuck R5=0 vertical adjustment (CRTC 1).**
    ...
    **RESOLVED BY AUTHOR CORRESPONDENCE, 2026-08-31.** English §11.2.4 p.85
    says adjustment increments C4 without considering R4. The specific R5=0 case in
    §11.3.2 pp.86–87 keeps the C4/R4 reset comparison active while adjustment persists.
...
Also noted while verifying (no answer needed, listed for completeness): p.196 places the
