...
|---|---|---|---|
| F1 | R10/R11 unreadable on types 0/1 | **confirmed** | §21.2.1/21.2.2 p.246: type 0 reads R12-R17, type 1 reads R14-R17 only, unrecognized regs read 0, reg 31 returns 127/255 on type 1. Q13 was resolved 2026-08-26 in favor of §21.2.2 with corroboration from the UM6845R datasheet and independent per-type table; ch.28 is the outlier. |
| F2 | Type 1 status bit 5 latched at C0=R0 | **confirmed** | §21.3.3 p.248: bit 5 = BORDER-R6 condition sampled only at C0=R0 (`false` = C4=C9=C0=0, `true` = C4=R6 ∧ C9=C0=0 at sample); bits 0-4/7 unused-read-0; R6=0-forced-border-while-C4>0 not reflected. All three F2 nuances verbatim. |
| F3 | Mid-line R7 write timing per type | **confirmed** | §16.4.1 pp.169-170 (blocked at C0vs<2, mechanism-2 latch, duration +(R0−C0vs), counter starts at next C0=0, PPI &36-active/&00-inactive at +6µs) and §16.4.2 p.170 (unconditional trigger, partial line counts, duration −(C0+1), PPI 5µs). Every F3 element near-verbatim. |
| F12 | Type-0 last-line/adjustment arbitration | **confirmed** | §10.3.1 pp.76-77, §11.2.2 pp.82-84, §12.2 pp.93-95, §13.2 pp.104-107: C0<2 window, C0==0 override, C0==1 break→adjustment with R5==0, C0==2 arbitration, R5>0-before-C0==3, completion re-establishing Last Line, R4/R9 write windows, exact-C0==R0 comparator switch. All confirmed. New: p.83 example 3 documents the **companion case** (R9 written in C0∈[2,R0−1] window → next line C4=38, C9=8) that t12 should encode beside the 39/8 exact-C0==R0 case. |
| F4 | Equality-only counter overflow | **confirmed** | §10.3 p.75 ("count to its maximum value (31) before going back to 0"), §12.1 p.93 ("C4 counts up to its limit (127) and loops back"), exception wording matches. p.79-80 tables (readable despite digest's smearing flag) corroborate. |
| F5 | Type-0 R0=0 freeze | **confirmed, digest prose errors nearby** | Freeze machinery, single deferred increment ("C4's last hiccup", §13.2.6 p.109), R4/R5/R9 ignored while R8 live, resume behaviour, and C9-vs-R5 lock persisting through C0=2 after unfreeze (p.107) all confirmed, including the full worked-vector table. Two adjacent digest-01 §8.1 sentences are wrong — see B2/B3; they do not affect the implemented t09/t16/t20 behaviour (which matches the source and `shaker-module-a-map.md`). |
| F6 | Type-0 spurious border byte (R1>R0) | **confirmed** | §17.6.2 pp.187: 0.5µs border byte raised at C0=R0, "BORDER OFF" on following character, any R0 incl. 0 (byte alternation), suppression via R8 SKEW-DISPTMG type-0-only, type 1/3/4 emit nothing (pp.187-188 "aggressive plagiarisms"). |
| F7 | RFD (CRTC 1) | **confirmed** | §11.6 pp.88-91: trigger (R5 0→nonzero exactly at C0==R0), two flags (VMA-source regardless of C4; parity management in the C9=R9-at-C0==R1 test), case-1/case-2 alternation, parity flip at C4=C9=C0=0 with R9 odd, IVM ON/OFF freezing (even C9 caveat), RFD#10/1-B taxonomy (UM6845R-8804T vs -8802T, 3 of 7 machines), recipes, HITACHI errata "real playground" remark (p.91, visually verified). Second trigger route §13.7.1.2 p.125 confirmed incl. R9-variant (line repetition) and R4-variant (stuck C4 on second frame). |
| F8 | Type-1 separate C5 counter | **confirmed** | §11.1 p.81 (C5/C9 dissociated on CRTCs 1/2; CRTC 0 has no C5), §11.2.3 p.84 worked table (C4 11→14 while C5 counts 0..15; VRAM pointer from C9), §11.3.2 p.86 R5=0 bug verbatim ("the state is not deactivated, C4 does not return to 0 and C5 loops"). F8's implementation description matches the source throughout. |
| F9 | R9 write at exact C0==R0 | **confirmed** | p.83: "The value of C9 is first compared to R9, causing C4 to increment… C9 is then compared to R5, and C9 is also incremented… we end up with C4==39 and C9==8." The digest's warning against modelling this as generic old/new-R9 sampling is consistent with the prose. |
...
| F11a-h | Minor/confirmatory | **confirmed** | HSYNC width semantics (§14.5), re-entrancy + infinite-VSYNC bypass (§16.3 p.168, vector values verbatim), R12/R13 readback (§21.2), dummy 31 (§21.4 p.250 incl. UM6845E distinction), VSYNC width R3h (§14.2), light-pen listing (§21.2 tables), skew bits (see B10), type-1 mid-row R12/R13 immediacy (§20.3.2 p.243 "loaded with R12/R13 **while C4=0**"; Living Daylights cited). F11i out of CRTC scope unchanged. |
...
1. **B1 — digest-02 §3 duration-table interpretation is wrong.** The slash pairs (e.g.
   ...
   the **NJIT** column ("I indicated a range of 2 values", p.134); the JIT column reads
   +0.25µs above NJIT (e.g. 2.3125/2.3750), consistent with the stated JIT delay. Retire the
   "digit-wrap corruption" ⚠ for p.134 (clean in pdftotext) and reinterpret the numbers.
2. **B2 — digest-01 §8.1 R0=0 stall accounting.** "Stall of N cycles ≈ losing N/(R9+1)
   ...
   (p.105), i.e. N/64 raster lines of display time. Q2 was resolved 2026-08-26: with R9=7,
...
3. **B3 — digest-01 §8.1 R0=1 description.** "Every 2µs frame chains into another 2µs
   ...
   R5=0 the adjustment "lasts 1 line of 2 µsec before ceasing" (p.105), giving a strictly
   **alternating** normal/additional pattern (p.108 case study, caption extracted verbatim on
   p.129). Replace with the alternating description; keep the R5-reach clause only for R5>0.
4. **B4 — digest-01 §4.2/F9: record the companion case.** p.83 example 3: R9 written inside
...
5. **B5 — digest-01 §4.3 omission:** an R9 write at C0==R0 does **not** cancel the
   VMA-from-R12/R13-while-C4==1 rule; only an R4(>0) write does (§11.2.4 note, p.85).
...
6. **B6 — digest-01 §5 omission:** disarm path when R1>R0 — if C0=R1 can never be met,
   C9==R9 alone deactivates the VMA-source state (p.88). Matters for F7.
7. **B7 — obsolete ⚠ VERIFY flags to retire** (text layer carries the content cleanly):
   digest-01 p.35 (VRAM bit table), p.79-80 (divergence tables); digest-02 p.131 (R3 layout),
   p.134 totals (see B1), p.145 aggregate totals (pdftotext only), p.167 trace (mostly
   legible); digest-03 §19.2.5 sub-case rules (prose readable; only cycle diagrams compressed),
   §21.3.3 transition rules (fully in prose), §17.4 p.183 JIT rule (plain prose; keep flag
   only for the p.184 worked table and p.186 deadline boundary).
...
10. **B10 — digest-03 §19.5.2 inversions/misattributions.** Even R9 ⇒ **even** total
    ...
    line-count N" parenthetical is inverted. The odd-C4 R8→3 imbalance note sits on p.207
    (digest cites p.208) and "self-correcting on subsequent frames" is not in the text —
    mark as digest inference (Q12).
11. **B11 — digest-03 §19.3 extra-line frame attribution.** Digest states the extra scanline
    ends "the odd frame's construction"; source pages conflict (p.199 "end of the first
    frame", p.206/217 "construction of the even frame", p.200 odd frame "inherits" it and
...
12. **B12 — digest-03 §28.1.8:** drop the "described as always-1 per §21.3" gloss (not in
    source; "always 1" bit 6 appears only in the CRTC3/4 STATUS-1 table, p.249). Q14 resolved
...
the compendium digests; corrections that fell out: the parity truth tables are pp.211-212
only, the IVM tables use R9=6 even, the p.186 deadline boundary was corrected, and p.248
alone carries the bit-5 diagrams. p.247 remains unverified.)
