...
| `plus/p1-crtc3-foundation` | Plus P1 CRTC3 counter/timing foundation in `rtl/plus/asic_video.v` (4 commits: register file + HCC `43bf460`, vertical chain `e14a7b0`, video pointer + DE/SKEW-DISPTMG `3d2b116`, HSYNC/VSYNC `bb1a509`) with vectors t01a-t04g. Review hardest: the >=R9 completion vs equality-only capture split (§10.3.4), the adjustment entry/end state machine incl. the t02e-caught C4-restart fix (§11.2.6/§11.3.3), the C4=0 ∧ C0=0 reload without a C9 term (§20.3.4 vs type 0's §20.3.1), the HSYNC entering-edge start convention (§15.2.2), and the infinite-HSYNC relation being static (W mod (R0+1) == 0) rather than dynamic — the dynamic variant was tried and produced unfaithful permanent chaining. Deliberate scope cuts to confirm: interlace stored-but-inert, read map/status = P5, pixel path + CPU/WAIT contract = P1 remainder. | Implemented, pending integration/review |
