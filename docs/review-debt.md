...
- **Plus P10a-P10f, P10h compatibility closure, 2026-08-29** — Scope: `Amstrad.sv`,
  ...
  **REMEDIATION IN PROGRESS — `plus/p10-hardware-remediation`, uncommitted.**
  The branch now shares the classic/Plus FDC decoder between production and its
  ...
  **SECOND-REVIEW REMEDIATION COMPLETE LOCALLY, uncommitted:** the three blockers
  ...
  **POST-REVIEW FINAL SCAN REOPENED CF-5, THEN REMEDIATED LOCALLY:** the scan
  ...
  snapshot apply did not wait for the FIFO/write tail. The current uncommitted
  delta adds a one-cycle ASIC-register reset at SNA start, captured one-cycle
