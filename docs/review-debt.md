...
**Status: one row open 2026-08-25** (`accuracy/f11h-and-ivm-vsync-coverage`, NOT CLEAR on
one blocking finding, remediation in flight on the same branch). All earlier rows were
cleared: the `accuracy/f10-fixtures` row cleared by the Claude Opus 5 review + remediation;
all others cleared 2026-08-24. The
...
| `accuracy/f11h-and-ivm-vsync-coverage` | F11h closure (same-edge R12/R13 write caught by the type-1 row-0 VMA reload, §20.3.2 p.242; vectors `t20j`/`t20k`), t24 type-1 IVM VSYNC positions from the p.208 table with the §19.8.2 p.225 alternation (vectors `t24a`-`t24c`), and a CI-only checkout bump. Review hardest: the p.242 shaded-cell semantics derived from the CRTC-0 contrast pair; the harness same-edge write phase in `t20j`/`t20k`; the p.208 box grid and the odd-C4-count alternation arithmetic behind the R4=6 fixture geometry; and the VSYNC fire/count-tick mux changes against the p.208 prose (MID-VSYNC on the ParityFrame-even frame). | **NOT CLEAR — Claude Opus 5 xhigh (ask-claude bridge, fresh session), 2026-08-25.** Record: `accuracy/f11h-t24-independent-review.md`. All four rule claims verified PDF-first and confirmed; no vector weakened; both soak re-mints justified. **B-1 (blocking)**: the t24 fix removed the type-1 IVM MID-VSYNC (§19.5.3 p.208 schedules it on the ParityFrame-even frame) — the count tick became `line_new` on both parities and no vector samples the half-line phase, while `f10-implementation-notes.md` claimed the residual narrowed. Non-blockings N-1..N-7 in the record. Remediation in flight on the branch: latched seam fire decision consumed at the half-line tick on the ParityFrame-even frame, keyed on ParityFrame; `t24c` pins the half-line phase. Reviewer sandbox could not execute gates or bite-tests; the parent reproduces them (logged in the remediation commit). |
