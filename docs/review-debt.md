...
| `plus/p0-parser-wiring` | Plus P0 wiring: A5 decisions + oversized-cbNN abort (`4556665`), `plus_mmu` cartridge windows + CPU read bridge + production service hookup (`7eaddf2`), CPR ioctl stream + boot integration bench (`5a69ebe`). Review hardest: classic-mode bit-identity of the new `oe`/WAIT/mux terms under `plus_mode=0`, the `plus_mmu` handshake/watchdog FSM, and the fail-closed parser paths now that they are load-bearing. | Implemented, merged into base (`daf1d6f`), pending review |
| `docs/split-differential-evidence` | Durable split-equivalence comparator and pinned run (`c68459b`). After integration beyond the split, `run.sh` was corrected to extract both frozen sides (`418aa68` pre-split and `2d4f880` reviewed split tip) so later intentional RTL changes cannot invalidate the provenance check. Review hardest: the two commit defaults, extraction paths, and that overriding either side remains explicit. | Original evidence reviewed as a pass-1 supplement; post-merge two-sided pin correction pending pass-2 review |
| `plus/p1-crtc3-foundation` | Plus P1 CRTC3 counter/timing foundation in `rtl/plus/asic_video.v` (rebased commits: register file + HCC `41117cf`, vertical chain `0a3f9b9`, video pointer + DE/SKEW-DISPTMG `ad522e3`, HSYNC/VSYNC `d185eb4`) with vectors t01a-t04g, plus the integration-audit fix for simultaneous C0=R1=R0 VMA'/VMA save/reload. Review hardest: that save/reload priority (§17.1/§17.6.1), the >=R9 completion vs equality-only capture split (§10.3.4), the adjustment entry/end state machine incl. the t02e-caught C4-restart fix (§11.2.6/§11.3.3), the C4=0 ∧ C0=0 reload without a C9 term (§20.3.4 vs type 0's §20.3.1), the HSYNC entering-edge start convention (§15.2.2), and the infinite-HSYNC relation being static (W mod (R0+1) == 0) rather than dynamic — the dynamic variant was tried and produced unfaithful permanent chaining. Deliberate scope cuts to confirm: interlace stored-but-inert, read map/status = P5, pixel path + CPU/WAIT contract = P1 remainder. | Implemented, pending integration/review |
