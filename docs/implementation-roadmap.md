...
- The current development state contains the accuracy/reference documents, the F1-F3 and main
  ...
  randomized equivalence soak (`make -C sim soak`, golden hash `0x654a244c2cce6e0b`), the
...
- `sim/` currently reports **177** required classic CRTC passes with no expected failures
  (verified 2026-08-31, Verilator 5.050); the soak reproduces golden hash
  `0x654a244c2cce6e0b`. The Plus leaf, MMU, SDRAM, and boot-integration suites are green.
