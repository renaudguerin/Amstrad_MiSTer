...
1. **U1 — `p1_video_test_top.v` `[production copy]` blocks: VERIFIED
   ...
   - Contract standing: p1a/p1b/p1c remain differential against an
     ...
     difference. **September 8 source correction:** `p1_mobo_bench`
     elaborates the real `Amstrad_motherboard` but ties `vram_din` to zero;
     it does not prove absolute production VRAM return/latch timing.
     No fixture/test change made (brief
