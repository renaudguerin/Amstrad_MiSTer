...
- **CPR loading**: new ioctl index for `.cpr`. RIFF parsing is trivial and sequential —
  ...
  independent).
- **CPR auto-reset & case-insensitivity (Phase P9, 2026-08-28)**: CPR parser accepts
  both `AMS!` and `Ams!` form-types and `CB00`/`cb00` chunk prefixes. The top-level reset
  logic in `Amstrad.sv` isolates `reset_base` for the parser/service, asserts system `reset`
