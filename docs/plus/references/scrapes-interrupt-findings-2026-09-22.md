...
## PRI no-wrap: evidence ranking

The user's recollection that CPCWiki is wrong is independently supported by the
existing [Copter 271 investigation](../../defects/copter271/README.md), section 4,
and the [archived status record](../../archive/current-status-history-2026-09-21.md),
“Plus PRI line compare, 2026-09-13” and “Hardware reports, September 12–13”.
Source commit `b5c3014f2844b5a932be8db074e5baef3faad43a` records the discriminator:
the title handler at cartridge offset `0x1852` alternates PRI `&FF` and `&37`;
the erroneous second `&37` match at line311 loaded the sky palette before the
next frame's logo. The no-wrap fix made `pr02` fail-before/pass-after, and the
later archived device observation confirms the logo fixed on `c595031` / `b5c3014`.
AmSpirit showed the intended palette switch 55 lines later than the faulty core.

**Accepted implementation policy: PRI does not wrap at 256.** The CPCWiki/Quasar
wrap prose is contradicted by the documented formula and this observed title
result; it is retained below as a source error/conflict, not an equally supported
policy option. The observed device acceptance covers the Copter title/palette
chain. It is not a fresh logic-analyser measurement of every unusual CRTC setup;
that limit does not reopen the accepted policy. PRI phase remains separate.
...
| I3: PRI range | S01 p.4 says PRI=10 repeats at 266; S22 p.8 agrees, with a visible-height qualification. S07 pp.1–2 describes wrap but gives the erroneous sum 255+8=263. | `asic_ga_timing.v:606` compares `{1'b0,pri}` to nine-bit `crtc_line`; motherboard line 477 supplies `{plus_vc[5:0],plus_rc[2:0]}`. It excludes ≥256 and adjustment, with no DE test. | Contradicted source claim; preserve accepted no-wrap policy backed by the Copter evidence above. Do not reopen it from this scrape. A genuinely new counter-configuration discrepancy needs its own hardware evidence. |
| I4: PRI phase | S01 p.4 says exactly HSYNC-start+10 µs independent of width. Existing digest §7 prefers revised Arnold's monitor-trailing-edge / 6 µs account. | `asic_ga_timing.v:604–608` fires on internal shaped monitor HSYNC falling, not on `crt_filter` output. | [B20-4 follow-up](b20-pri-phase-2026-09-23.md) freshly verifies revised Arnold §2.4 (archived revision 115989). The connected-module R3 sweep follows its width-dependent 6-cycle clamp, with one master clock of IRQ edge latency; widths 1–2 have no shaped pulse/PRI. This contradicts the fixed +10 µs account but cannot adjudicate original hardware. Short-width and line-boundary semantics remain open; no RTL change. |
...
polarity. The original Copter investigation and subsequent archived device observation were
also re-read to establish why no-wrap outranks the contradictory scrape prose. No simulation,
