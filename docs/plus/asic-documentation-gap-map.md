...
| `&6800` PRI | Programmable raster line; zero selects classic 52-line source; 9-bit compare `{0,PRI} == {VC5..VC0,RC2..RC0}` per Arnold-revision §2.4, so lines 256 and above never match (the CPCWiki ASIC page's "PRI=10 also fires at 266" contradicts that formula; Copter 271's title and AmSpirit agree with the formula) | `rtl/plus/asic_regs.v`, `rtl/plus/asic_ga_timing.v` | `sim/plus/asic_pri_test.cpp`, `sim/plus/asic_ga_timing_diff_test.cpp`, `sim/plus/asic_regs_test.cpp` | **partial**: counter, arbitration and interrupt path are owned; the source conflict between a 6 µs and approximately 10 µs fire offset remains a hardware discriminator. |
...
| Model capabilities: RAM size, FDC, tape, keyboard/joypad matrix, ROM page 7 rule | `rtl/plus/plus_model_select.v`, motherboard gates, `plus_mmu` | b6_menu_mask, P8, P10 input/boot/FDC fixtures | **partial**: internal selectors and principal gates are owned; full-top FDC/tape and real keyboard/title behavior remain validation boundaries. |
