...
before the next screenshot request. This prepares the host workflow without a
MiSTer; no device execution or hardware acceptance has been demonstrated.
...
its paths and input. It is a template: its Enter key does **not** select a known
...
Input accepts ASCII alphanumerics, MBC navigation keys `UDLROEHFM`, `:XX` taps,
...
Three captures follow **one** load/input sequence. Inspect them for tearing,
