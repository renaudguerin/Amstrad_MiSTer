...
**Source review CLEAR; local acceptance gates passed.** Reviewed code is committed
at `94725cc` on `general/b4-csl-ssm` over `8731452`; `SSM_SAMPLE_RECORDER` remains undefined by default.
...
The review stage performed no synthesis, CI dispatch, device run or physical-memory
write. Subsequent stream-finish authorization covers commit, integration, push and the
normal default-off build. It does not close enabled-recorder synthesis or device gates.
...
Source `94725cc` merged without conflicts onto `master` at `501cfbc`; all 20 reviewed
source hashes remained identical. The merged checkout passed full `make -C sim`,
183 host tests, lint and soak `0xb1cb70da95c2e44f`. The full merged run includes all
five composition cases. The only executable integration adjustment raises the CI
simulation timeout from 30 to 60 minutes after prior run `34684600995` exhausted it
during B8 field tests; test commands and assertions are unchanged. Synthesis-policy
and timing-parser self-tests pass. Local gate logs are retained under ignored
`docs/references/csl-ssm-integration-2026-09-12/`.
