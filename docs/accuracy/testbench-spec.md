...
| t02l-t02r type-0 natural VSYNC requires C0=2 qualification; steady R0=1 blocks C4=R7, while qualified exact-C0=0 writes to R0=0/1 retain the documented frozen/two-character outcomes; `t02o` treats blocked-comparison consumption as an inferred hardware discriminator, and `t02p`-`t02r` pin type-1 isolation plus snapshot/live-type lifecycle reconstruction — implemented, hardware validation pending | ACCC v1.11 English §16.4.1.2 pp.168-169 and §§16.4.2/19.3.2.1; author confirmation applies to the short-line block and dynamic R0 outcomes, not `vsync_allow` consumption | Round-2 consequence audit and lifecycle review |
