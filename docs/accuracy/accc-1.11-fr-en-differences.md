...
| BL-007 | §4.4.3; 26/26 | French's practical 1 µs-early placement applies specifically to `OUT(C),r8` and separately exempts `OUTI` on CRTC3/4. English broadens it to generic write-I/O and adds `OUTD` to the exception. | Author response 2026-08-31: `OUTD` is intentional. Keep the published difference until a future edition is released; B11/Plus timing still needs its own evidence. |
| BL-008 | §7.2; 40–41/39–40 | The synchronization routine differs: French uses `19968-21` and derives 5 µs as `1+1+3`; English uses `19968-23`, derives `2+1+3`, then subtracts 1 µs. | Author response: align on `19968-21`, chosen for faster VSYNC acquisition. Programming guidance only; future edition alignment is unpublished. |
| BL-009 | §4.4.4; 27–31/27–30 | French attributes `/WAIT` request/repetition to the Gate Array; English says Gate Array/ASIC. | Author response: the ASIC uses the same `/WAIT` request/repetition mechanism. Treat as Plus evidence, not proof of current RTL timing. |
...
| BL-014 | §11.6; 89–90/87 | French defines exactly two RFD states and says the VMA-source state persists independently of a C0/R1 test; English says “several” and gives a different persistence clause. | Author response: C0=R1 does not clear the VMA-source state; C9=R9 does, and parity can make that equality fail. Existing F17 language follows this route; code changes still require a failing vector. |
...
| BL-022 | §14.1; 132/130 | French says HSYNC “begins” when C3l reaches R3l; English says “ends.” French conflicts with its own definitions, tables, and later prose. | Author confirms the French typo: reaching C3l=R3l ends HSYNC. Current behavior stands; promised French correction is unpublished. |
...
| BL-033 | §16.4.1.2; absent/169 | English alone adds two normative R0/VSYNC cases: R0>2→0 can make VSYNC infinite, and →1 with R3h=1 yields 2 µs. | Author confirms the paragraphs are normative and intended for French. Audit premise and add failure-first vectors before RTL; retain English p.169 as the published v1.11 anchor. |
...
| BL-037 | §19.3.4; 202/201 | French warns that complete-interlace construction must account for VSYNC at C4=R7; English replaces it with a normative frame-start-only R8 activation rule. | Author confirms the French advice is wrong and R8 should be modified at frame start. Preserve mid-frame robustness coverage without treating it as the normative construction recipe. |
| BL-038 | §19.5.3; 209/208 | French explicitly sets `ParityC9=ParityFrame` at type-1 frame start; English omits it. | Author explicitly confirms the assignment at every frame start. IA-2/`t32a` already implement and pin it; no further RTL action. |
...
| BL-043 | §22; absent/250 | English alone adds a vague warning that small/sync-period writes to other registers may affect other registers, without an actionable condition or result. | Author intends to restore it to French but calls it low-importance. Keep as non-operational guidance, not a test oracle; future addition is unpublished. |
