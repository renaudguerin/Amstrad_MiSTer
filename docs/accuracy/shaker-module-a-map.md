...
| **(O)** R1 STORIES | 8 | §17 pp.175–187, §17.6 pp.185–186 | **Presence/type/skew implemented, width/phase blocked as F13.** Stage 1 (`accuracy/a3-f6-stage1`, t10a-t10e) emits a full-character type-0 DE gap; type 1 none. Stage 2 rendered 1 µs (16 mode-2 px). Stage 2b visual reading of ACCC pp.186/195 assigns the documented 0.5 µs pulse to the CRTC DE phase; test/production clock phase and both GA paths agree. SHAKER checkpoint: compare seam against Logon System photos (expected 8 mode-2 px) and capture the type-0 DE pin if possible (expected low only for C0=R0's second byte). No production change before that evidence. |
