...
1. **Implemented (2026-08-28, N5)**: fail the synthesis job in `build.yml` and `local-build.yml`
   when the compile log contains `Implicit Net warning` (Warning 10236) — catches this exact class
   in any synthesized file for one line of policy script.
2. **Queued (Full)**: elaborate `Amstrad.sv` under Verilator in the Tier A lint chain with
